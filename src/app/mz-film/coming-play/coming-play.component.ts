...
export class ComingPlayComponent implements OnInit {
  ...
  ngOnInit() {
    this.loading=true;
    let promise = this.getData.getData(`http://localhost:3000/api/flim/ComPlaying?page=1&count=5`);
    let _this=this;
    // 2、需要使用的时候在写一下代码
  ...
  srolls(){
    ...
    let bigBox=document.getElementById('mzComingPlay');
    let smallBox=document.getElementById('mzComingPlayUl');
  ...
  setScrollY(){
    ...
    var box=document.getElementById('mzComingPlay');
